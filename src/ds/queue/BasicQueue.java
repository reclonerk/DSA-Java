...
public class BasicQueue <X> {
    ...
    public X deQueue(){
        ...
        // otherwise if this is the last item on queue, the queue needs to get reset to empty.
        else if(front == end){
            item = data[front];
            data[front] = null;
            front = -1;
    ...
    public X access(int position){
        if(size() == 0 || position > size()){
            throw new IllegalStateException("No items in the queue or the position is greater than size.");
        }
